...
class HelpAgent(
    ...
    override suspend fun handle(
        ...
        val systemPrompt = """
            ...
            4. **Состояние Git**: Для вопросов о текущих изменениях, ветках или истории используйте `git branch`, `git status`, `git diff` и `git log`.
            ...
            2. **Комбинирование инструментов**: Если вопрос сложный (например, "Как работает авторизация в текущей ветке?"), сначала вызовите `searchKnowledgeBase` для поиска документации, затем `git branch` для проверки ветки, а затем найдите и прочитайте файл реализации.
        ...
        return sendMessage(
            context,
            systemPrompt,
            request.copy(query = request.query.substringAfter("/help "))
        )
    }
}
