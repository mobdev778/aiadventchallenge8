...
class GetStickyFactsUseCase(
    ...
    suspend fun invoke(messages: List<ChatMessage>): ChatMessage? {
        ...
        val prompt =
            "Ты — модуль управления долговременной памятью (Sticky Facts) для LLM-ассистента.\n" +
                    "Твоя задача — извлечь из диалога ТОЛЬКО критически важные факты, которые необходимы для продолжения работы, когда старые сообщения удалятся из контекста.\n" +
                    "\n" +
                    "КРИТИЧЕСКИЕ ПРАВИЛА:\n" +
                    "1. ИГНОРИРУЙ приветствия, вежливость, пустой флуд и мета-комментарии (например: 'Привет', 'Чем помочь', 'Понял', 'Секунду').\n" +
                    ...
                    "3. Если в тексте НЕТ новых критически важных фактов, задач или технических деталей, НЕ ВЫВОДИ ВООБЩЕ НИЧЕГО. Твой ответ должен быть абсолютно пустым. Никаких фраз вроде 'Нет фактов' или 'Sticky_Facts: нет'. Строго 0 символов.\n" +
                    "4. Формат вывода: Строго пары КЛЮЧ:ЗНАЧЕНИЕ, разделенные переносом строки. Без кавычек вокруг ключей, если это не нужно. Никакого текста от себя.\n" +
                    "\n" +
                    "Пример 1 (Есть факты):\n" +
