...
/**
 * Компоненты для отображения блоков кода с подсветкой синтаксиса Kotlin на экране чата.
 *
 * Данный файл содержит визуальный компонент [CodeFenceBlock], который стилизует
 * переданный фрагмент кода под «огороженный» блок (code fence) с тёмным фоном,
 * моноширинным шрифтом и базовой синтаксической раскраской ключевых слов, литералов
 * и комментариев Kotlin. Также предоставляется возможность скопировать содержимое
 * блока в буфер обмена по нажатию на иконку.
 */
...
/**
 * Composable-компонент, отображающий блок кода с подсветкой синтаксиса Kotlin.
 *
 * Блок имеет тёмный фон, скруглённые углы и моноширинный шрифт. В правом верхнем углу
 * расположена иконка копирования, при нажатии на которую текст кода помещается в системный
 * буфер обмена. Подсветка синтаксиса выполняется на уровне построения [AnnotatedString] и
 * охватывает ключевые слова Kotlin, строковые/символьные/числовые литералы, регулярные
 * выражения, аннотации и однострочные комментарии.
 *
 * @param code Исходный код на языке Kotlin, который необходимо отобразить и раскрасить.
 * @param modifier Модификатор компоновки, применяемый к корневому контейнеру блока.
 *   По умолчанию используется [Modifier.Companion].
 */
@Composable
fun CodeFenceBlock(
    code: String,
...
/**
 * Внутренняя модель, описывающая сегмент кода, подлежащий подсветке.
 *
 * @param start Начальный индекс (включительно) совпадения в исходной строке кода.
 * @param endExclusive Конечный индекс (исключительно) совпадения в исходной строке кода.
 * @param color Цвет, которым должен быть раскрашен данный сегмент.
 */
