...
/**
 * Содержимое экрана добавления профиля.
 *
 * Компонент отрисовывает форму для создания нового профиля, включающую
 * текстовые поля для имени и содержимого (формат Markdown), кнопку «Назад»
 * для возврата к предыдущему экрану и кнопку «Добавить» для сохранения.
 * Взаимодействие с UI осуществляется через события [AddProfileScreenEvent],
 * текущее состояние берётся из [AddProfileScreenState].
 *
 * Для улучшения восприятия на форме присутствуют заголовок [ScreenHeader] и
 * подписи к полям ввода, выполненные с помощью стандартных текстовых компонентов.
 * Поле содержимого поддерживает многострочный ввод и вертикальную прокрутку.
 *
 * @param state Иммутабельное состояние экрана, содержащее текущие значения полей ввода.
 * @param onEvent Лямбда-обработчик, вызываемый при возникновении пользовательских действий
 *                (нажатия кнопок, изменение текста).
 */
@Composable
